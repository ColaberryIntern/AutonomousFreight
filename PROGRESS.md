...
- [x] Closed the D33 corpus gap using Karun's repo, with no dependency on Karun, GCP, or the ShipCES mailbox
  ...
  - Follow-up (same day): added `scripts/corpus/buildCorpusReport.ts`, which renders the harness results as a self-contained HTML page (hero figure, KPI row, single-series horizontal bar chart with hover + keyboard tooltips, the two defect callouts, three rule-chosen worked examples showing real email beside what we extracted, and the full 35-row table). It runs the SAME `parseEml` to `parseEmailToRfq` chain as the harness and re-verifies every SHA-256 before scoring, so the page cannot drift from the test. Palette validated with the dataviz validator: the project's red `#e53e3e` against amber `#dd6b20` FAILED the normal-vision floor (ΔE 9.2, below 15), so amber was dropped entirely; the shipped set is single-hue sequential blue `#2b6cb0` plus status green `#38a169` (light) and `#3785cf` / `#2fa571` (dark), all checks PASS against both surfaces. Status is always glyph plus text label, never color alone. Output is `~/Downloads/ShipCES-Corpus-Accuracy.html`, marked CONFIDENTIAL: it embeds real customer email excerpts, so it is written locally and deliberately never published to a hosted URL. A dash guard aborts the build if our own prose contains an em/en-dash, while exempting verbatim quoted customer text.
